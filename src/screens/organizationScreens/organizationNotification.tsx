...
const OrganizationNotification: React.FC<OrganizationNotificationProps> = ({
  navigation,
}) => {
  // Other variables
  ...
  const dataList: Notification[] =
    data?.getAllNotificationsByUser?.map((item: any) => ({
      ...
      isRead: item.isRead,
      mobileRedirect: item.mobileRedirect.id,
    })) ?? [];
      ...
      onPress={() => {
        notificationRead({
          ...
          onCompleted: async data => {
            handleNotificationRedirection(item);
            countRefetch();
          },
          onError: error => {
            dispatch(
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={{flex: 1, backgroundColor: COLORS.DashboardBackgroundColor}}>
        {renderDeleteModal()}
        <OrganizationHeader title={Strings.NOTIFICATION} showBackArrow={true} />
        <OrganizationSubHeader />
